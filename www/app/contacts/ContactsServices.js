...
//        console.warn(result);
    ...
    var getCountOfNonFollowers = function (db, callBack) {
      var selectQuery = "SELECT count (*) FROM contact WHERE  status != 'selected' and status !='deleted' and id NOT IN ( SELECT c.id from contact  c where c.email ='' and c.phone_1 ='' and c.phone_2 ='' and c.first_name ='' and c.last_name ='') ";
      try {
        $cordovaSQLite.execute(db, selectQuery).then(function (result) {
          //  console.warn(result);
          return callBack(result);
...
//      timeout : 5000,
        ...
        if (contacts instanceof Array) {
          getContactbyId(db, contacts[i].id, function (result) {
            ...
            if (result.rows.length > 0) {
              //delete photo local
              cameraService.deleteFileUploaded(result.rows.item(0).photofilelocation, function () {
        ...
        downloadAndOverride(contactArray[counter].id, function (urlImg, i) {
          updateContactPhoto(db, contactArray[counter].id, urlImg, function () {
            ...
            if (contactArray[counter].list.indexOf('Import') != -1) {
              ...
              saveOrUpdateContactTel(db, contactArray[counter], {
                email: contactArray[counter].email,
                phone_1: contactArray[counter].phone_1,
                phone_2: contactArray[counter].phone_2
              }, function () {
                //  alert("synchron");
                recurssiveImgForSynchro(db, ++counter, contactArray, callBack);
              }, function () {
    ...
    var geolocalicationAdress = function (db, contact, callBack) {
      console.log('geolocation addrsee')
      if (/Android|BlackBerry Mini/i.test(navigator.userAgent)) {
        console.log('android')
    ...
    var saveContactInTel = function (contact, callBack) {
      if (contact.phone_1.length == 0 && contact.phone_2.length == 0) {
        return callBack();
      } else if (contact.first_name.length == 0 && contact.last_name == 0) {
        return callBack();
      } else {
        ...
        if(contact.email == $translate.instant('loading.data') && contact.photofilelocation !== "img/photo_top_title.jpg" )
          ...
          note =  removeSlashes(contact.actu) + " \n" + contact.comment;
    ...
    var updateContactDevice = function (contactTel, contact, oldEmailPhone, callBack) {
      if (contact.phone_1.length == 0 && contact.phone_2.length == 0) {
        return callBack()
      } else if (contact.first_name.length == 0 && contact.last_name.length == 0) {
        return callBack()
      } else {
        var dateBirthday = null;
        if (/Android|BlackBerry Mini/i.test(navigator.userAgent)) {
        ...
        if (contact.firstsendemail.trim() == "" && contact.meeting_point.trim() == "") {
          ...
          if (contactDevice.note == "" || contactDevice.note == null) {
            ...
            if(contact.email == $translate.instant('loading.data') && contact.photofilelocation !== "img/photo_top_title.jpg" )
                ...
                contactDevice.note = removeSlashes(contact.actu) + "\n" + contact.comment ;
...
//
     ...
     * get address with gps position
     */
    var getPositionAndAdressePosition = function (db, contact, callBack) {
      cordova.plugins.diagnostic.isLocationAuthorized(function (authorized) {
        console.log("Location is " + (authorized ? "authorized" : "unauthorized"));
        if (authorized) {
          var lat = 0;
          ...
          $cordovaGeolocation
            ...
            .then(function (position) {
              ...
              if (latlng.lat != 0 && latlng.lng != 0) {
                ...
                SynchroServices.getAddressFromCoordonates(lat, lng, contact.id, function (address) {
                  callBack(address);
                });
              } else {
                callBack('');
        ...
        if (authorized) {
          ...
          $cordovaGeolocation
            ...
            .then(function (position) {
              ...
              console.log('Coordonnées GPS du position actuelle: ' + latlng.lat + " : " + latlng.lng);
    ...
    /**
     * getAdress from coordinates
     */
    var coordinatesToAddress = function (lat, long, callBack) {
      ...
      $http(getAddressRequest).success(function (response, status, headers, config) {
        if ("OK" == response.status)
          return callBack(response.results[0].formatted_address);
